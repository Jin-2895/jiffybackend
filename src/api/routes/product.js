...
export default (app) => {
  ...
  route.post(
    "/add",
    general.tokenDecrypt,
    passport.authenticate("jwt", { session: false }),
    general.searchUser,
    upload.array("uploaded_file", 5),
    middlewares.validation.postProduct,
    async (req, res, next) => {
      try {
        let input = req.body;
  ...
  route.post(
    "/view",
    middlewares.validation.viewProducts,
    async (req, res, next) => {
      try {
        let input = req.body;
        const model = Container.get("Products");
  ...
  route.get(
    "/view/:id",
    middlewares.validation.viewSingleProduct,
    async (req, res, next) => {
      try {
  ...
  route.put(
    ...
    passport.authenticate("jwt", { session: false }),
    general.searchUser,
    middlewares.validation.deleteProduct,
    async (req, res, next) => {
      try {
        let input = {};
  ...
  route.put(
    ...
    passport.authenticate("jwt", { session: false }),
    general.searchUser,
    middlewares.validation.updateProduct,
    async (req, res, next) => {
      try {
        let input = req.body;
        input.id = req.params.id;
